...
# Script de test pour l'exercice 013 : Afficher un nombre (recursif)
# Usage: ./test_ex013.sh
...
echo -e "${BLUE}=== Test de l'exercice 013 : Afficher un nombre (recursif) ===${NC}"
...
int main(void)
{
    // Test de la fonction pw_putnbr avec differents nombres
    pw_putnbr(42);
    write(1, "\n", 1);  // Ajout d'un retour a la ligne pour separer
...
if [ "$OUTPUT_VISIBLE" = "$EXPECTED_OUTPUT" ]; then
    echo -e "${GREEN}✅ Test reussi! La fonction affiche correctement tous les nombres${NC}"
    TEST_RESULT=0
else
...
# Test individuel pour verifier le comportement avec 42
...
if [ $? -eq 0 ]; then
    ...
    if [ "$SINGLE_OUTPUT" = "42" ]; then
        ...
        echo -e "${RED}❌ Test individuel echoue - Sortie: '$SINGLE_OUTPUT' (attendu: '42')${NC}"
        TEST_RESULT=1
    fi
else
...
if [ $? -eq 0 ]; then
    ...
    if [ "$ZERO_OUTPUT" = "0" ]; then
        echo -e "${GREEN}✅ Test avec 0 reussi${NC}"
    else
        echo -e "${RED}❌ Test avec 0 echoue - Sortie: '$ZERO_OUTPUT' (attendu: '0')${NC}"
        TEST_RESULT=1
    fi
else
...
if [ $? -eq 0 ]; then
    ...
    if [ "$ONE_OUTPUT" = "1" ]; then
        echo -e "${GREEN}✅ Test avec 1 reussi${NC}"
    else
        echo -e "${RED}❌ Test avec 1 echoue - Sortie: '$ONE_OUTPUT' (attendu: '1')${NC}"
        TEST_RESULT=1
    fi
else
...
if [ $? -eq 0 ]; then
    ...
    if [ "$MAX_OUTPUT" = "999999999" ]; then
        echo -e "${GREEN}✅ Test avec UINT_MAX reussi${NC}"
    else
        echo -e "${RED}❌ Test avec UINT_MAX echoue - Sortie: '$MAX_OUTPUT' (attendu: '999999999')${NC}"
        TEST_RESULT=1
    fi
else
...
# Test avec un nombre a 3 chiffres
...
if [ $? -eq 0 ]; then
    ...
    if [ "$NINE_OUTPUT" = "999" ]; then
        echo -e "${GREEN}✅ Test avec 999 reussi${NC}"
    else
        echo -e "${RED}❌ Test avec 999 echoue - Sortie: '$NINE_OUTPUT' (attendu: '999')${NC}"
        TEST_RESULT=1
    fi
else
...
if [ $? -eq 0 ]; then
    ...
    if [ "$LONG_OUTPUT" = "123456789" ]; then
        echo -e "${GREEN}✅ Test avec 123456789 reussi${NC}"
    else
        echo -e "${RED}❌ Test avec 123456789 echoue - Sortie: '$LONG_OUTPUT' (attendu: '123456789')${NC}"
        TEST_RESULT=1
    fi
else
...
if [ $? -eq 0 ]; then
    ...
    if [ "$POWERS_OUTPUT" = "10 100 1000" ]; then
        echo -e "${GREEN}✅ Test puissances de 10 reussi${NC}"
    else
        echo -e "${RED}❌ Test puissances de 10 echoue - Sortie: '$POWERS_OUTPUT' (attendu: '10 100 1000')${NC}"
        TEST_RESULT=1
    fi
else
...
if [ $EXEC_STATUS -eq 0 ] && [ $TEST_RESULT -eq 0 ]; then
    echo -e "\n${GREEN}✅ Exercice 013 valide avec succes${NC}"
    echo -e "${GREEN}La fonction affiche correctement tous les nombres en utilisant la recursion!${NC}"
else
    echo -e "\n${RED}❌ Exercice 013 non valide${NC}"
    exit 1
fi
